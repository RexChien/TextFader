...
var TextFader = TextFader || (function (fader) {
    ...
    me.createFadeStr = function (output, openTagStart, color, openTagEnd, str, closeTag) {
        var ret = '';
        switch (output) {
            case 'carrd':
                ret = '[' + str + ']{#' + color + '}';
                break;
            case 'console':
                ret = '\u001B[38;2;' + parseInt(color.substr(0, 2), 16) +';' + parseInt(color.substr(2, 2), 16) +';' + parseInt(color.substr(4, 2), 16) + 'm' + str + '\u001B[0m'
                break;
            default:
        ...
        switch (type) {
            case 'vertical':
                ...
                for (ii = 0; ii < len; ii++) {
                    ...
                    ret += lineStart + me.createFadeStr(output, openTagStart, retColors[ii], openTagEnd, txtLines[ii], closeTag) + lineEnd;
                }
                ...
                for (ii = 0; ii < len; ii++) {
                    ...
                    for (jj = 0; jj < len2; jj++) {
                        if (txtLines[ii].substr(jj, 1) === ' ' && (output === 'vbulletin' || output === 'forum')) {
                            ...
                            ret += me.createFadeStr(output, openTagStart, retColors[cIndex], openTagEnd, txtLines[ii].substr(jj, 1), closeTag);
                        }
                        cIndex++;
                    }
                    ret += lineEnd;
                ...
                for (ii = 0; ii < len; ii++) {
                    ...
                    for (jj = 0; jj < len2; jj++) {
                        if (txtLines[ii].substr(jj, 1) === ' ' && (output === 'vbulletin' || output === 'forum')) {
                            ...
                            ret += me.createFadeStr(output, openTagStart, retColors[ii][cIndex], openTagEnd, txtLines[ii].substr(jj, 1), closeTag);
                        }
...
// parse json
let params = JSON.parse(myArgs[0]);
async function main() {
    // read file
    params.txt = await fs.readFile(params.txt, 'utf8');
    try {
        // if file exist
        await fs.access('./result.txt');
        // delete it
        await fs.unlink('./result.txt');
    } catch (err) {
    }
    // write file
    await fs.writeFile('./result.txt', TextFader.fade(params));
}
